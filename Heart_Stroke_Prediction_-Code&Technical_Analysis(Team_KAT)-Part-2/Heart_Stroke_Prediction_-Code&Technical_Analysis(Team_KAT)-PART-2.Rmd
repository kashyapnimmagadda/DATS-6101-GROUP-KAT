...
# table(test_data$stroke)
# 
print(paste("Number of observations in test data split: ", nrow(test_data)))
...
The coefficients table shows the estimated effect of each predictor on the log odds of having a stroke. For example, for every one-unit increase in "sysBP", the log odds of having a stroke increase by 0.0125. The intercept represents the log odds of having a stroke when all the predictors are equal to zero.
...
The AUC (Area Under the Curve) of the ROC (Receiver Operating Characteristic) curve is 0.725, indicating that the model has a moderate ability to distinguish between stroke and non-stroke cases.
...
Based on the confusion matrix and performance metrics, it appears that the decision tree model has classified all positive cases correctly but has incorrectly classified a significant number of negative cases. The model has achieved a high accuracy of 84.82%, but this is mainly due to the class imbalance in the data, where there are many more negative cases than positive cases.

The precision and recall metrics are also quite high, with a precision of 84.82% and recall (sensitivity) of 100%. However, the specificity is very low at 0%, indicating that the model is not very good at identifying negative cases. The F1 score, which is the harmonic mean of precision and recall, is also quite high at 91.78%.

The AUC-ROC curve shows that the model is not able to differentiate well between positive and negative cases, with an AUC value of 0.5 However, this may be due to the class imbalance in the data, and it is important to consider other performance metrics as well.
...
After tuning the decision tree model, we obtained an accuracy of 77.18%, which is lower than the accuracy of the original decision tree model (84.82%). The precision value of the tuned decision tree model is also 86.55%, which is the same as the original model. However, the sensitivity and specificity values are 86.55% and 24.87%, respectively, which are lower than the corresponding values of the original model (100% and 0%). The F1 score is also 86.55%, the same as the precision and recall values.

The confusion matrix shows that the tuned decision tree model misclassified more stroke patients than the original model. The tuned model predicted 193 false negatives (patients who have had a stroke but were predicted as not having a stroke) compared to the original model's zero false negatives. Additionally, the tuned model predicted 145 false positives (patients who have not had a stroke but were predicted as having a stroke) compared to the original model's zero false positives.

The ROC curve of the tuned decision tree model has an AUC value of 0.67, which is lower than the AUC value of the original model (0.81). This indicates that the tuned model is less effective in distinguishing between stroke and non-stroke patients than the original model.

## Comparison of decision tree models

In the first decision tree model, we have used the default rpart function to fit the decision tree on the stroke dataset. The accuracy of the model was found to be 0.7747.

On the other hand, in the second decision tree model, we have tuned the parameters using the rpart.control function and then fit the decision tree on the stroke dataset. The accuracy of this model was found to be 0.8442. This indicates that the tuned decision tree model is performing better than the default decision tree model.

The confusion matrix shows that the tuned decision tree model has 1071 true negatives, 191 false positives, 7 false negatives, and 2 true positives. The specificity of the model is 0.01036, and the sensitivity of the model is 0.99351. The precision of the model is 0.84865, and the recall of the model is 0.99351. The F1 score of the model is 0.91538.

The ROC curve of the tuned decision tree model has an AUC value of 0.672498.
...
The Random Forest model has an accuracy of 85.02%, which is slightly better than the Decision Tree model. The feature importance plot shows that the most important features are sysBP, age, totChol, and diaBP. These features are also in the top five features for importance in the Decision Tree model.

The AUC-ROC curve for the Random Forest model has an area under the curve (AUC) of 0.685, which is slightly better than the AUC of the Decision Tree model. This suggests that the Random Forest model may be a better overall model for predicting stroke risk in this dataset.
...
The model has an overall accuracy of 85.05%, which means that 85.05% of the predictions made by the model were correct. The precision of the model is 85.46%, which means that when the model predicts that a person is at risk of stroke, it is correct 85.46% of the time. The recall (sensitivity) of the model is 99.26%, which means that the model correctly identifies 99.26% of people who are actually at risk of stroke. The specificity of the model is 5.7%, which means that the model correctly identifies only 5.7% of people who are not at risk of stroke. The F1 score of the model is 91.85%, which is a weighted average of precision and recall and is often used as a more balanced measure of model performance.

### Model building 3: Logistic Regression with Randform forest feature selection.
```{r glm_model_org, echo = TRUE}

# Fit a logistic regression model to the training data
glm_model_org <- glm(stroke ~ diaBP + sysBP + age + glucose + BMI + totChol +
                               cigsPerDay + heartRate + prevalentHyp + currentSmoker, 
                 family = binomial, 
                 data = train_data)


print(summary(glm_model_org))

# Make predictions on the testing data
glm_preds <- predict(glm_model_org, 
                      newdata = test_data, 
                      type = "response")
...
The accuracy of the logistic regression model on the test set is 85.44%. The precision of the model is 85.52%, meaning that out of all the predicted stroke cases, 85.52% are actually true positive cases. The recall (sensitivity) of the model is 99.72%, meaning that the model correctly identifies 99.72% of the actual positive cases. The specificity of the model is 5.7%, meaning that the model correctly identifies only 5.7% of the actual negative cases. The F1 score of the model is 92.08%, which is a weighted harmonic mean of the precision and recall. Overall, the model seems to perform well in identifying the actual stroke cases, but has a low specificity, meaning it incorrectly identifies a large number of negative cases as positive.

```{r glm_model_AUC_curve, echo = TRUE}

prob_glm <- predict(glm_model_org, 
                test_data,
                type = "response")
...
```{r glm_full_model_eval_metrics_org, echo = TRUE}
