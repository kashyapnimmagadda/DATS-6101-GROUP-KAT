...
# train_idx <- createDataPartition(stroke_df$stroke, p = 0.67, list = FALSE)
...
# train_data_balanced <- SMOTE(stroke ~ ., train_data, perc.over = 100, k = 5)
# test_data_balanced <- SMOTE(stroke ~ ., test_data, perc.over = 100, k = 5)
...
#   geom_bar(aes(fill = ..count..)) +
#   scale_fill_gradient("Count", low = "#F0E442", high = "#999999") +
#   labs(x = "stroke", y = "Count of People", title = "Distribution of Target in Train")

```


```{r}
# ggplot(test_data_balanced, aes(x = stroke)) +
#   geom_bar(aes(fill = ..count..)) +
#   scale_fill_gradient("Count", low = "#F0E442", high = "#999999") +
#   labs(x = "stroke", y = "Count of People", title = "Distribution of Target in Test")

```
# Note:
## Combining the over-sampled data with the original data: 
Once the process of over-sampling the training data is complete, combining it with the original data to increase the number of observations.



```{r bind_ovdata_orgdata, echo = TRUE}

# stroke_df_balanced <- rbind(train_data_balanced, stroke_df[-train_idx, ])
# stroke_df_balanced <- rbind(test_data_balanced, stroke_df[train_idx, ])

```


```{r re_sampling_SMOTE, echo = TRUE}

# Check the balance of the target variable in the original data
table(stroke_df$stroke)

# Perform oversampling using the ROSE package
stroke_df_balanced <- ROSE(stroke ~ ., 
                         data = stroke_df, 
                         seed = 123, 
                         N = nrow(stroke_df) + sum(stroke_df$stroke == 1))$data

# stroke_df_balanced <- stroke_df_balanced_og
# Check the balance of the target variable in the oversampled data
table(stroke_df_balanced$stroke)

```

The above code returns the balanced dataframe using over-sampling technique which equalize the matches of target variable categories.

## Summary after data-transforming
```{r summary_after_data_transformation}

# To get the summary statistics of the balanced dataset

xkablesummary(stroke_df_balanced,
  title = "Table : Statistical Summary after balancing dataframe",
  pos = "center",
  bso = "hover"
)
...
```{r balanced_target_var_plot, echo = TRUE}
...
ggplot(stroke_df_balanced, aes(x = stroke)) +
  geom_bar(aes(fill = ..count..), stat = "count") +
  geom_text(aes(label = ..count..), stat = "count", vjust = -0.5) +
  scale_fill_gradient("Count", low = colors[1], high = colors[2]) +
...
```
```{r}
stroke_df_balanced.data = glm(formula = stroke ~ ., 
                              data = stroke_df_balanced, 
                              family = binomial)

print(summary(stroke_df_balanced.data))
```
...
train_index <- sample(nrow(stroke_df_balanced), 0.7 * nrow(stroke_df_balanced))
...
library(caret)
confusionMatrix(predictions > 0.5, test$stroke)
