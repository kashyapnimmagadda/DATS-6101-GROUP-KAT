...
Before applying SMOTE, there were 2516 observations with stroke=0 and 451 observations with stroke=1 in the training data. After applying SMOTE to balance the classes, there were 1353 observations with stroke=0 and 1804 observations with stroke=1 in the balanced training data.

The total number of observations in the balanced training data is 3157, which is higher than the original training data because SMOTE creates synthetic data points to balance the classes.
...
rf_model <- randomForest(stroke ~ ., data = train_data_balanced, importance = TRUE)
...
tree_model <- rpart(stroke ~ ., data=train_data_balanced, method="class")
...
After applying SMOTE to balance the classes in the test data, the resulting number of observations in each class is 579 and 772 for stroke = 0 and stroke = 1 respectively. There are a total of 1351 observations in the test data after SMOTE.
...
The intercept has a negative estimate of -15.207006, which means that for a person with all predictor variables equal to zero, the log odds of having a stroke is -15.207006.
...
sysBP
glucose
The coefficients of these variables have positive values, except for prevalentStroke, which has a positive coefficient value, indicating that as the values of these variables increase, the likelihood of having a stroke also increases.
...
From the VIF values, we can see that the highest VIF is for sysBP at 3.46, indicating that it has moderate collinearity with other predictors in the model. However, all the VIF values are below the commonly accepted threshold of 5, indicating that multicollinearity is not a major concern in this model.
...
From this confusion matrix, we can see that the model predicted 1071 true negatives, 14 true positives, 179 false negatives, and 7 false positives.
...
Based on the confusion matrix, we can calculate several performance metrics:

Accuracy: the proportion of correctly classified observations over the total number of observations. The accuracy of the full model is 85.37%, which means that 85.37% of the stroke cases were correctly classified.

Precision: the proportion of true stroke cases over all predicted stroke cases. The precision of the full model is 85.68%, which means that 85.68% of the predicted stroke cases were actually stroke cases.

Recall (Sensitivity): the proportion of true stroke cases over all actual stroke cases. The recall of the full model is 99.35%, which means that 99.35% of the actual stroke cases were correctly identified by the model.

Specificity: the proportion of true non-stroke cases over all actual non-stroke cases. The specificity of the full model is 7.25%, which means that only 7.25% of the actual non-stroke cases were correctly identified by the model.

F1 Score: the harmonic mean of precision and recall. The F1 score of the full model is 92.01%, which provides a balanced view of the model's performance in terms of both precision and recall.
