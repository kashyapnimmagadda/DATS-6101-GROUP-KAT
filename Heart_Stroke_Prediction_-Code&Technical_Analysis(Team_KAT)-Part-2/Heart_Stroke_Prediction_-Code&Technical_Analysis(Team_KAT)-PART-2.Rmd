...
head(stroke_df, n = 2)
```{r head_4_csv, include=TRUE, echo=TRUE}
...
dat$gluc.cat <-
  factor(dat$gluc.cat,
    levels = c("Below 60", "60 - 90", "90 - 120", "120 - 180", "180 - 273", "Above 273")
  )
dat$age.cat <-
  factor(dat$age.cat,
...
ggplot(stroke_1, aes(x = BMI, fill = stroke)) +
  ...
  geom_density(alpha = 0.3, linetype = "dashed", size = 1.2) +
  geom_vline(xintercept = mean(stroke_1$BMI), linetype = "dotted", size = 1.2) +
...
ggplot(stroke_df, aes(x = age, fill = stroke)) +
  ...
  geom_vline(xintercept = mean(stroke_df$age), linetype = "dashed", linewidth = 1.2) +
  ggtitle("Density plot for age of people who had stroke") +
  theme_bw() +
  theme() +
  scale_fill_discrete(name = "stroke", labels = c("0 - No", "1 - Yes"))
...
```{r logit_corr_model_confusion_matrix, echo = TRUE}

# create confusion matrix
Prediction <- factor(test_preds_acc, levels = c(0, 1))
Actual <- factor(test_data$stroke, levels = c(0, 1))
full_model_confusion_mat <- confusionMatrix(Prediction, Actual)

# print the confusion matrix
full_model_confusion_mat$table
...
### Logistic regression model evaluation metrics with highly correlated variables 
...
```{r logit_full_model_confusion_matrix, echo = TRUE}
...
```{r full_model_with_feature_importance, echo = TRUE}

full_model_step <- step(log_full_model, direction = "backward", trace = 0)
summary(full_model_step)

# Make predictions on the testing data
predictions_step <- predict(full_model_step, 
                      newdata = test_data, 
                      type = "response")


predictions_step_acc <- ifelse(predictions_step > 0.5, 1, 0)

```

This output shows the result of a logistic regression model where the dependent variable is "stroke" and the independent variables are "gender", "age", "cigsPerDay", "prevalentStroke", "prevalentHyp", "sysBP", and "glucose". The output shows the estimates of the regression coefficients, their standard errors, z-values, and p-values. The intercept term is significantly different from zero, as indicated by the p-value of less than 2e-16. The other variables are also significant predictors of the occurrence of a stroke, as their p-values are less than 0.05. The deviance residuals are also shown, which indicate the goodness-of-fit of the model. The null deviance and residual deviance values show that the model has explained a significant amount of the variation in the data. The AIC value is also shown, which is a measure of the model's goodness-of-fit, where a lower value indicates a better fit.


```{r full_model_step_evaluation, echo=TRUE}

# create confusion matrix
confusion_mat_full_step <- confusionMatrix(factor(predictions_step_acc, levels = c(0, 1)), 
                               factor(test_data$stroke, levels = c(0, 1)))

# print the confusion matrix
confusion_mat_full_step$table


# print accuracy
full_model_step_accuracy <- confusion_mat_full_step$overall['Accuracy']
print(paste("Accuracy: ", round(full_model_step_accuracy, 4) * 100, "%"))


# print precision
full_model_step_precision <- confusion_mat_full_step$byClass['Pos Pred Value']
print(paste("Precision: ", round(full_model_step_precision, 4) * 100, "%"))


# print recall (sensitivity)
full_model_step_sensitivity <- confusion_mat_full_step$byClass['Sensitivity']
print(paste("Recall (Sensitivity): ", round(full_model_step_sensitivity, 4) * 100, "%"))


# print specificity
full_model_step_specificity <- confusion_mat_full_step$byClass['Specificity']
print(paste("Specificity: ", round(full_model_step_specificity, 4) * 100, "%"))


# print F1 score
full_model_step_f1_score <- 2 * (confusion_mat_full_step$byClass['Pos Pred Value'] * confusion_mat_full_step$byClass['Sensitivity']) / (confusion_mat_full_step$byClass['Pos Pred Value'] + confusion_mat_full_step$byClass['Sensitivity'])


print(paste("F1 Score: ", round(full_model_step_f1_score, 4) * 100, "%"))
...
```{r full_model_ROC_curve, echo =TRUE}
...
# plot the ROC curve
plot(roc_object, main = "ROC Curve", 
