...
After applying SMOTE to balance the classes in the test data, the resulting number of observations in each class is 752 and 772 for stroke = 0 and stroke = 1 respectively. There are a total of 1524 observations in the test data after SMOTE.
...
plot(ROC_dt_tune)
paste("Area under curve of Tuned Decision Tree: ", auc(ROC_rf))
...
text(x = 0.7, y = 0.2, labels = paste("AUC = ", round(auc_val_tuned_dt_org, 3)), col = "red", cex = 1.5, font = 2)

paste("Area under curve of Tuned Decision Tree: ", auc_val_tuned_dt_org)

```

### Model building - 2: Random Forest for original dataset
...
prob_log_step_glm <- predict(glm_full_model, 
                test_data,
                type = "response")
